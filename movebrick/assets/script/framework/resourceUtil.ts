import { _decorator, Prefab, Node, SpriteComponent, SpriteFrame, Rect, ImageAsset, resources, error, Texture2D, instantiate, isValid, find, loader, Color, log, TextAsset, JsonAsset } from "cc";
 ...
 */
    ...
    public static loadMatRes(matPath: string) {
        return new Promise((resolve, reject) => {
            this.loadRes(`${matPath}`, Prefab, (err: any, mat: Prefab) => {
                if (err) {
                    console.error('effect load failed', matPath);
                    reject && reject();
                    return;
                }
    ...
    public static getJsonData(fileName: string, cb: Function) {
        this.loadRes("datas/" + fileName, null, function (err: {}, content: JsonAsset) {
            if (err) {
                error(err.message || err);
                return;
            }
    ...
    public static getTextData(fileName: string, cb: Function) {
        this.loadRes("datas/" + fileName, null, function (err: {}, content: TextAsset) {
            if (err) {
                error(err.message || err);
                return;
            }
